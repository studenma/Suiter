...
def input_json_structure_validator(json_content):
    ...
    * 't-way' - the stregth of a T-way on test sequence level 
    ...
    for idx in range(test_case_array_length):
        ...
        # for element in json_content['test_sequence'][idx]['method']['values']:
        #     if element not in allowed_methods:
        #         """
        #         if there is some value which is not in list of allowed http methods
        #         -> there could be method passed as a varaible
        #         -> but, the type of this value has to be string and there can be only one element in whole list
        #         """
        #         if (type(element) is not str) and (len(json_content['test_sequence'][idx]['method']['values']) != 1):
